...
# Writes the output in the txt file
orig_stdout = sys.stdout
f = open('futoshiki_output.txt', 'w')
sys.stdout = f
if status == cp_model.FEASIBLE:
    print(solver.Value(elements[0]), ",", solver.Value(elements[1]), ",",  solver.Value(elements[2]), ",",  solver.Value(elements[3]))
    print(solver.Value(elements[4]), ",", solver.Value(elements[5]), ",",  solver.Value(elements[6]), ",",  solver.Value(elements[7]))
    print(solver.Value(elements[8]), ",", solver.Value(elements[9]), ",",  solver.Value(elements[10]), ",",  solver.Value(elements[11]))
    print(solver.Value(elements[12]), ",", solver.Value(elements[13]), ",",  solver.Value(elements[14]), ",",  solver.Value(elements[15]))
